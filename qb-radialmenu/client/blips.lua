...
RegisterNetEvent("qb-radialmenu:client:toggletattoo", function()
	local coords = GetEntityCoords(PlayerPedId())
	local closest = 1500
	local closestLocation

	for k,v in pairs(Config.Tattoooshops) do
		local dist = GetDistanceBetweenCoords(coords, v)
		if dist < closest then
			closest = dist
			closestLocation = v
...
RegisterNetEvent("qb-radialmenu:client:togglemarket", function()
	local coords = GetEntityCoords(PlayerPedId())
	local closest = 1500
	local closestLocation
